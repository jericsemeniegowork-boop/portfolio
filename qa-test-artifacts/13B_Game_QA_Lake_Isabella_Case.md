...
|---|---|---:|---|
| TC-LAKE-001 | Verify treasure reward is granted once | Critical | Reward and inventory integrity |
| TC-LAKE-002 | Verify reconnect does not duplicate reward | Critical | Network persistence |
| TC-LAKE-003 | Verify session change does not restore claimed reward | Critical | Session state |
| TC-LAKE-004 | Verify area reload does not reset claimed chest state | High | World streaming regression |
| TC-LAKE-005 | Verify multiple treasure maps do not corrupt inventory state | High | Inventory regression |
...
- Frontier Trails Online Patch v1.8.0 is used as the sample fictional title-update reference for this portfolio sample.
...
| PC crash / launch / HDR / FSR changes | Compatibility / Performance / Stability | Launch, graphics settings, crash logs, and supported PC checks pass. |
...
| 2 | Enter stable on mountback. | Stable menu loads within expected threshold. |
